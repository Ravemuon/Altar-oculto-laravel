...
<div class="container">
    <div class="text-center my-4">
        ...
        <div class="d-flex justify-content-center mb-5 gap-3 flex-wrap">
            <a href="{{ route('dashboard.index') }}" class="btn btn-umbanda btn-lg shadow">📊 Dashboard</a>
        ...
        {{-- Produtos agrupados por categoria (Carousel) --}}
                                        ...
                                        @foreach($chunk as $produto)
                                            @php
                                                $imgSrc = $produto->imagem
                                                    ? (Str::startsWith($produto->imagem, ['http://','https://'])
                                                        ? $produto->imagem
                                                        : asset('storage/'.$produto->imagem))
                                                    : 'https://via.placeholder.com/500x400.png?text='.$produto->nome;
                                            @endphp
                                            <div class="col-12 col-md-4 mb-4">
                                                <div class="card card-umbanda text-center h-100 shadow-sm">
                                                    <img src="{{ $imgSrc }}" alt="{{ $produto->nome }}" class="card-img-top" style="height: 450px; object-fit: cover; border-radius: 0.5rem 0.5rem 0 0;">
                            ...
                            @php
                                $imgSrc = $produto->imagem
                                    ? (Str::startsWith($produto->imagem, ['http://','https://'])
                                        ? $produto->imagem
                                        : asset('storage/'.$produto->imagem))
                                    : 'https://via.placeholder.com/100x100.png?text='.$produto->nome;
                            @endphp
                            <tr>
                                <td>
